...
-- Create real-time payment status table
CREATE TABLE public.payment_status_updates (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Enable RLS
ALTER TABLE public.payment_status_updates ENABLE ROW_LEVEL SECURITY;

-- Create policies
CREATE POLICY "System can manage payment status updates"
ON public.payment_status_updates
FOR ALL
USING (true);
...
-- Create function to log payment status change
...
CREATE TRIGGER payment_processing_status_change
  AFTER UPDATE ON public.payment_processing
  FOR EACH ROW
